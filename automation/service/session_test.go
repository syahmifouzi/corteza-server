...
func TestSession_Start(t *testing.T) {
	...
	_, err = ses.Start(g, types.SessionStartParams{Invoker: auth.Anonymous()})
	req.EqualError(err, "could not find starting step")

	g.AddStep(wfexec.NewGenericStep(nil))
	_, err = ses.Start(g, types.SessionStartParams{StepID: 4321, Invoker: auth.Anonymous()})
	...
	_, err = ses.Start(g, types.SessionStartParams{StepID: 42, Invoker: auth.Anonymous()})
