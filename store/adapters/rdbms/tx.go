...
func (s *Store) Tx(ctx context.Context, fn func(context.Context, store.Storer) error) error {
	...
	return txHandler(ctx, s.DB, s.TxRetryLimit, s.TxRetryErrHandler, func(ctx context.Context, tx SQLExecer) error {
...
//
func txHandler(ctx context.Context, dbc interface{}, max int, reh txRetryOnErrHandler, task func(context.Context, SQLExecer) error) error {
	var (
		lastTaskErr error
		err         error
		db          *sqlx.DB
