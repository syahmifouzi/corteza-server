...
type (
	...
	DataPrivacyAPI interface {
		...
		SensitiveModuleList(context.Context, *request.DataPrivacySensitiveModuleList) (interface{}, error)
	}

	// HTTP API interface
	DataPrivacy struct {
		SensitiveDataList   func(http.ResponseWriter, *http.Request)
		SensitiveModuleList func(http.ResponseWriter, *http.Request)
	}
)
...
func NewDataPrivacy(h DataPrivacyAPI) *DataPrivacy {
	return &DataPrivacy{
		...
		SensitiveModuleList: func(w http.ResponseWriter, r *http.Request) {
			defer r.Body.Close()
			params := request.NewDataPrivacySensitiveModuleList()
			if err := params.Fill(r); err != nil {
				api.Send(w, r, err)
				return
			}

			value, err := h.SensitiveModuleList(r.Context(), params)
			if err != nil {
				api.Send(w, r, err)
				return
			}
...
func (h DataPrivacy) MountRoutes(r chi.Router, middlewares ...func(http.Handler) http.Handler) {
	r.Group(func(r chi.Router) {
		...
		r.Get("/data-privacy/sensitive-module", h.SensitiveModuleList)
	})
}
